...
function PageInner() {
  ...
  async function addSelectedToBox() {
    if (!clientId) return;
    const chosen = eligible.filter((r) => selected[r.id]);
    setErrMsg("");
    setOkMsg("");
    // Reglas: Amazon requiere foto
    const amazonNoPhoto = chosen.filter(r => r.carrier === "Amazon" && !r.photoUrl);
    if (amazonNoPhoto.length) {
      setErrMsg(`No se pueden agregar ${amazonNoPhoto.length} tracking(s) de Amazon sin foto.`);
      return;
    }
    ...
    const newItemIds = Array.from(new Set([...(box.itemIds || []), ...chosen.map((r) => r.id)]));
    ...
    chosen.forEach((r) => {
      const inRef = doc(db, "inboundPackages", r.id);
  ...
  async function closeBox() {
    ...
    setClosing(true);
    try {
      const boxRef = doc(db, "boxes", openBox.id);
    ...
    <main className="p-4 md:p-8 space-y-6">
      ...
      <section className="space-y-2">
        ...
        <div className="grid gap-2">
          {eligible.map((r) => (
            <label key={r.id} className="border rounded p-3 flex items-center gap-3">
              {r.photoUrl ? (
                <>
              ...
              <div className="flex-1">
                ...
                <div className="text-xs text-neutral-500">
                  Recibido: {new Date(r.receivedAt).toLocaleString()}
                </div>
              </div>
            </label>
          ))}
